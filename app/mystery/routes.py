...
def serialize_state(user_key):
    game_day = get_game_date("mystery")
...
def reveal():
    ...
    if attempt["completed"]:
        return jsonify({"ok": False, "message": "Mystery Monday is already complete.", "state": serialize_state(user_key)}), 409
    ...
    revealed_count += 1
    save_mystery_attempt(
        user_key, game_date, revealed_count,
        load_json_list(attempt, "guesses_json"), False, False, 0,
    )
...
def guess():
    ...
    guesses = load_json_list(attempt, "guesses_json")
    revealed_count = int(attempt["revealed_count"])
    won = is_correct_answer(answer, puzzle)
    guesses.append({"answer": answer, "correct": won, "clue": revealed_count})

    if won:
        score = score_for_clues(revealed_count)
        completed = True
    ...
    save_mystery_attempt(
        user_key, game_date, revealed_count, guesses, completed, won, score,
    )

    stats = None
    if completed:
        row = finalize_game_stats(user_key, "mystery", game_date, score, won)
